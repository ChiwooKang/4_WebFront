...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    const product = {
        ...
        'information' : function(){ // 객체의 기능
            ...
            // 해당 객체의 변수(key)가 아닌
            // 전역변수 선택된다
            // 왜?  같은 객체의 변수(key)를 선태갛는 방법이 별도 존재해서
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    stdList.push( new Student(2, 3, "김길동") );
    stdList.push( new Student(3, 4, "박길동") );
    stdList.push( new Student(4, 5, "장길동") );




    // for of : 배열 / 컬렉션 오소 반복 접근용 for 문
...
function Student(grade, ban, name){
    ...
    // 생성자 함수에서의 this 
    //== new 연산자를 통해 만드러지는 객체(앞으로 만들어질 객체)
...
document.getElementById("btn3").addEventListener("click",function(){
    ...
    // JSON.pares(JSON 문자열) : JSON문자열 - > JS객체 반환
