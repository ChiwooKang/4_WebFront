...
inputPw.addEventListener("keyup", function(){
    ...
    if (inputPw.value == inputPwConfirm.value) {
        ...
        //pwMessage.style.color = "green" ; // 직접 css 추가 방법
...
inputName.addEventListener("change", function(){
    ...
    // - 형식이 일치할 경우
    // "이름" 입력창 오른쪽에 "한글자만 입력하세요" 글자를 빨간색으로 출력

    else{
        nameMessage.innerText="한글자만 입력하세요";
...
function validate(){
    ...
    // 라이도/체크박스.checked : 체크되면 true / 아니면 false
    if (!gender[0]&& !gender[1].checked) {
        alert("성별을 선택해주세요");
        return false;
    }
    */
